...
export CFLAGS="${CFLAGS} -Ofast -mcpu=cortex-a53 -ftree-vectorize -funsafe-math-optimizations"
export CXXFLAGS="${CXXFLAGS} -Ofast -mcpu=cortex-a53 -ftree-vectorize -funsafe-math-optimizations"
...
make_target() {
  ...
  cp ${PKG_BUILD}/projects/unix/mupen64plus-video-rice.so \
     ${PKG_BUILD}/projects/unix/mupen64plus-video-rice-base.so
  ...
  case ${DEVICE} in
    AMD64|RK3588|S922X|RK3399)
      ...
      make -C projects/unix all ${PKG_MAKE_OPTS_TARGET}
      cp ${PKG_BUILD}/projects/unix/mupen64plus-video-rice.so \
         ${PKG_BUILD}/projects/unix/mupen64plus-video-rice-simple.so
    ;;
  esac
}
...
makeinstall_target() {
  ...
  cp ${PKG_BUILD}/projects/unix/mupen64plus-video-rice-base.so \
     ${ULIBDIR}/mupen64plus-video-rice.so
  chmod 0644 ${ULIBDIR}/mupen64plus-video-rice.so

  if [ -e "${PKG_BUILD}/projects/unix/mupen64plus-video-rice-simple.so" ]
  then
    cp ${PKG_BUILD}/projects/unix/mupen64plus-video-rice-simple.so \
       ${ULIBDIR}
    chmod 0644 ${ULIBDIR}/mupen64plus-video-rice-simple.so
  fi

  mkdir -p ${USHAREDIR}
  cp ${PKG_BUILD}/data/RiceVideoLinux.ini \
     ${USHAREDIR}/RiceVideoLinux.ini
