...
makeinstall_target() {
  ...
  cp ${PKG_BUILD}/projects/unix/mupen64plus-audio-sdl-base.so \
     ${ULIBDIR}/mupen64plus-audio-sdl.so
  chmod 0644 ${ULIBDIR}/mupen64plus-audio-sdl.so

  if [ -e "${PKG_BUILD}/projects/unix/mupen64plus-audio-sdl-simple.so" ]
  then
    cp ${PKG_BUILD}/projects/unix/mupen64plus-audio-sdl-simple.so \
       ${ULIBDIR}
    chmod 0644 ${ULIBDIR}/mupen64plus-audio-sdl-simple.so
  fi
}