...
make_target() {
  ...
  cp ${PKG_BUILD}/projects/unix/mupen64plus-video-glide64mk2.so \
     ${PKG_BUILD}/projects/unix/mupen64plus-video-glide64mk2-base.so
  ...
  case ${DEVICE} in
    AMD64|RK3588|S922X|RK3399)
      ...
      make -C projects/unix all ${PKG_MAKE_OPTS_TARGET}
      cp ${PKG_BUILD}/projects/unix/mupen64plus-video-glide64mk2.so \
         ${PKG_BUILD}/projects/unix/mupen64plus-video-glide64mk2-simple.so
    ;;
  esac
}
...
makeinstall_target() {
  ...
  cp ${PKG_BUILD}/projects/unix/mupen64plus-video-glide64mk2-base.so \
     ${ULIBDIR}/mupen64plus-video-glide64mk2.so
  chmod 0644 ${ULIBDIR}/mupen64plus-video-glide64mk2.so

  if [ -e "${PKG_BUILD}/projects/unix/mupen64plus-video-glide64mk2-simple.so" ]
  then
    cp ${PKG_BUILD}/projects/unix/mupen64plus-video-glide64mk2-simple.so \
       ${ULIBDIR}
    chmod 0644 ${ULIBDIR}/mupen64plus-video-glide64mk2-simple.so
  fi

  mkdir -p ${USHAREDIR}
  cp ${PKG_BUILD}/data/Glide64mk2.ini \
     ${USHAREDIR}/Glide64mk2.ini
